...
function ZoneTab({ label, count, active, onClick, color }) {
  return (
    <button
      onClick={onClick}
      className={`group flex items-center gap-1 rounded-lg border px-1.5 py-1.5 text-[14px] font-bold transition focus:outline-none focus:ring-2 focus:ring-[#f59e0b]/30 ${
        active
    ...
    >
      ...
      <span>{label}</span>
      <span
        className={`rounded-full px-1 py-0.5 text-[12px] font-bold leading-none ${
